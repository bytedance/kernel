		    ...
		    struct ext4_map_blocks *map, int flags)
{
	...
	down_read(&EXT4_I(inode)->i_data_sem);
	if (ext4_test_inode_flag(inode, EXT4_INODE_EXTENTS)) {
		retval = ext4_ext_map_blocks(handle, inode, map, flags & EXT4_EX_FILTER);
	} else {
		retval = ext4_ind_map_blocks(handle, inode, map, flags & EXT4_EX_FILTER);
	}
	if (retval > 0) {
		unsigned int status;
...
static inline void ext4_truncate_page(struct inode *inode,
				       loff_t start, loff_t end)
{
	...
	page = find_lock_page(inode->i_mapping, start >> PAGE_SHIFT);
	if (!page)
		return;
					 ...
					 loff_t start, loff_t end)
{
	...
	if (!IS_ALIGNED(start | end, PAGE_SIZE) &&
		...
		ext4_truncate_page(inode, start, min(page_boundary, end));
		if (end > page_boundary)
			ext4_truncate_page(inode,
		 ...
		 struct iattr *attr)
{
	...
	if (attr->ia_valid & ATTR_SIZE) {
		...
		if (attr->ia_size != inode->i_size) {
			/* attach jbd2 jinode for EOF page tail zeroing */
			...
			/*
			 * Update c/mtime and tail zero the EOF page on
