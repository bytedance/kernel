				   ...
				   u64 attr_valid, bool update_cmtime)
{
	...
	if (update_cmtime || !S_ISREG(inode->i_mode)) {
			    ...
			    u64 attr_valid, u64 attr_version)
{
	 ...
	 * inode.
	 *
	 * But with wb_trust_server, if all the following conditions are met,
	 ...
	 */
	if (!is_wb || wb_update || !S_ISREG(inode->i_mode))
	...
	if ((!is_wb || wb_update) && S_ISREG(inode->i_mode)) {
		if (oldsize != attr->size) {
			...
			if (!fc->explicit_inval_data)
				inval = true;
		} else if (!fc->wb_trust_server && fc->auto_inval_data) {
			/*
			 * When fc->wb_trust_server is set, the old_mtime
			       ...
			       int error)
{
	...
	if (error || arg->major != FUSE_KERNEL_VERSION)
		...
		if (arg->minor >= 6) {
			...
			if (arg->flags & FUSE_READDIR_PAGES) {
				fc->readdir_pages =
					min_t(unsigned int,
					      FUSE_MAX_READDIR_PAGES,
					      max_t(unsigned int,
						    arg->readdir_pages, 1));
