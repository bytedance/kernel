...
static void init_new_hugetlb_page(struct hstate *h, struct page *page)
{
	INIT_LIST_HEAD(&page->lru);
	set_compound_page_dtor(page, HUGETLB_PAGE_DTOR);
	hugetlb_set_page_subpool(page, NULL);
...
static void prep_and_add_allocated_page(struct hstate *h,
					struct list_head *page_list)
{
	unsigned long flags;
				...
				nodemask_t *node_alloc_noretry)
{
	...
	for_each_node_mask_to_alloc(h, nr_nodes, node, nodes_allowed) {
		...
		page = only_alloc_fresh_huge_page(h, gfp_mask, node, nodes_allowed,
						node_alloc_noretry);
		if (page)
			return page;
	}
