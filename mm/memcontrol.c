...
bool memcg_oom_priority_enable __read_mostly;
...
static void do_flush_stats(void)
{
	 ...
	 */
	if (!cgroup_rstat_flush_ratelimited(root_mem_cgroup->css.cgroup))
		return;
	...
	if (cgroup_subsys_on_dfl(memory_cgrp_subsys)) {
		if (mem_cgroup_swappiness(memcg))
			max += min(READ_ONCE(memcg->swap.max),
					(unsigned long)get_total_swap_pages());
	} else { /* v1 */
		if (mem_cgroup_swappiness(memcg)) {
			/* Calculate swap excess capacity from memsw limit */
			unsigned long swap = READ_ONCE(memcg->memsw.max) - max;

			max += min(swap, (unsigned long)get_total_swap_pages());
		}
	}
	return max;
}
...
static bool obj_stock_flush_required(struct memcg_stock_pcp *stock,
				     struct mem_cgroup *root_memcg)
{
	struct obj_cgroup *task_objcg = READ_ONCE(stock->task_obj.cached_objcg);
	struct obj_cgroup *irq_objcg = READ_ONCE(stock->irq_obj.cached_objcg);
	struct mem_cgroup *memcg;

	if (in_task() && task_objcg) {
		memcg = obj_cgroup_memcg(task_objcg);
		if (memcg && mem_cgroup_is_descendant(memcg, root_memcg))
			return true;
	}
	if (irq_objcg) {
		memcg = obj_cgroup_memcg(irq_objcg);
		if (memcg && mem_cgroup_is_descendant(memcg, root_memcg))
			return true;
	}
