...
static bool in_kernel_space(unsigned long address)
{
	/*
	 ...
	 * address space.
	 */
	if (IS_ENABLED(CONFIG_X86_64) && (address & PAGE_MASK) == VSYSCALL_ADDR)
		return false;
...
{
	return in_kernel_space(regs->di) && !in_kernel_space(regs->si);
}
