						   ...
						   blk_mq_req_flags_t flags)
{
	...
	if (rq_list_empty(plug->cached_rq)) {
		if (plug->nr_ios == 1)
			return NULL;
		rq = blk_mq_rq_cache_fill(q, plug, op, flags);
		if (rq)
			goto got_it;
		return NULL;
	}
	...
	if (blk_mq_get_hctx_type(op) != rq->mq_hctx->type)
		return NULL;
	if (op_is_flush(rq->cmd_flags) != op_is_flush(op))
		return NULL;
		...
		blk_mq_req_flags_t flags)
{
	...
	rq = blk_mq_alloc_cached_request(q, op, flags);
	if (!rq) {
		struct blk_mq_alloc_data data = {
			.q		= q,
			.flags		= flags,
...
static void blk_complete_request(struct request *req)
{
	 ...
	 */
	if (blk_crypto_rq_has_keyslot(req) && total_bytes >= blk_rq_bytes(req))
...
static void __blk_execute_rq_nowait(struct gendisk *bd_disk, struct request *rq, bool at_head,
...
{
	__blk_execute_rq_nowait(bd_disk, rq, at_head, done, true);
}
		...
		int at_head)
{
	 ...
	 */
	rq->end_io_data = &wait;
	__blk_execute_rq_nowait(bd_disk, rq, at_head, blk_end_sync_rq,
			!blk_rq_is_poll(rq));
