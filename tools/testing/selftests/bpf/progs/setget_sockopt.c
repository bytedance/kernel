...
//	{ .opt = IPV6_TCLASS, .new = 0xe1, .expected = 0xe1, .tcp_expected = 0xe0, },
	{ .opt = IPV6_TCLASS, .new = 0xe1, .expected = 0xe1, .tcp_expected = 0xe1, },
...
static int bpf_test_socket_sockopt(int i, void *ctx, struct sock *sk)
{
	const struct sockopt_test *t;
	...
	if (t->flip)
		...
		return bpf_test_sockopt_flip(ctx, sk, t, SOL_SOCKET);

	//return bpf_test_sockopt_int(lc->ctx, lc->sk, t, SOL_SOCKET);
	return bpf_test_sockopt_int(ctx, sk, t, SOL_SOCKET);
}

//static int bpf_test_ip_sockopt(__u32 i, struct loop_ctx *lc)
static int bpf_test_ip_sockopt(int i, void *ctx, struct sock *sk)
{
	const struct sockopt_test *t;
	...
	if (t->flip)
		...
		return bpf_test_sockopt_flip(ctx, sk, t, IPPROTO_IP);

	//return bpf_test_sockopt_int(lc->ctx, lc->sk, t, IPPROTO_IP);
	return bpf_test_sockopt_int(ctx, sk, t, IPPROTO_IP);
}

//static int bpf_test_ipv6_sockopt(__u32 i, struct loop_ctx *lc)
static int bpf_test_ipv6_sockopt(int i, void *ctx, struct sock *sk)
{
	const struct sockopt_test *t;
	...
	if (t->flip)
		...
		return bpf_test_sockopt_flip(ctx, sk, t, IPPROTO_IPV6);

	//return bpf_test_sockopt_int(lc->ctx, lc->sk, t, IPPROTO_IPV6);
	return bpf_test_sockopt_int(ctx, sk, t, IPPROTO_IPV6);
}

//static int bpf_test_tcp_sockopt(__u32 i, struct loop_ctx *lc)
static int bpf_test_tcp_sockopt(int i, void *ctx, struct sock *sk)
{
	const struct sockopt_test *t;
	//struct sock *sk;
	//void *ctx;
...
#pragma clang loop unroll(full)
	for (n = 0; n < ARRAY_SIZE(sol_socket_tests); n++) {
		if (bpf_test_socket_sockopt(n, ctx, sk)) {
			n++;
			break;
		}
	}
...
#pragma clang loop unroll(full)
		for (n = 0; n < ARRAY_SIZE(sol_tcp_tests); n++) {
			if (bpf_test_tcp_sockopt(n, ctx, sk)) {
				n++;
				break;
			}
		}
...
#pragma clang loop unroll(full)
		for (n = 0; n < ARRAY_SIZE(sol_ip_tests); n++) {
			if (bpf_test_ip_sockopt(n, ctx, sk)) {
				n++;
				break;
			}
		}
...
#pragma clang loop unroll(full)
		for (n = 0; n < ARRAY_SIZE(sol_ipv6_tests); n++) {
			if (bpf_test_ipv6_sockopt(n, ctx, sk)) {
				n++;
				break;
			}
		}
