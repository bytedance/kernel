...
static bool vmas_contain_fsdax(struct vm_area_struct **vmas,
			       long npages)
{
	size_t i;

	for (i = 0; i < npages; ++i) {
		if (vma_is_fsdax(vmas[i]))
			return true;
	}
...
static void put_pages(struct page **pages, long npages)
{
	size_t i;
			  ...
			  struct page **pages)
{
	...
	if (mm == current->mm) {
		 ...
		 * interface.
		 */
		if (ret > 0 && vmas_contain_fsdax(vmas, ret)) {
			put_pages(pages, ret);
			ret = -EOPNOTSUPP;
		}
		kvfree(vmas);
