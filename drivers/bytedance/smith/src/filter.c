...
#define ADD_EXECVE_EXE_SHITELIST 70         /* F */
#define DEL_EXECVE_EXE_SHITELIST 89         /* Y */
#define DEL_ALL_EXECVE_EXE_SHITELIST 113    /* q */
#define EXECVE_EXE_CHECK 122                /* z */
#define PRINT_ALL_WHITELIST 42              /* * */
#define ADD_EXECVE_ARGV_SHITELIST 74        /* J */
#define DEL_EXECVE_ARGV_SHITELIST 92        /* \ */
#define DEL_ALL_EXECVE_ARGV_SHITELIST 110   /* n */
#define EXECVE_ARGV_CHECK 125               /* } */

#define ADD_WRITE_NOTIFI 87                 /* W */
#define DEL_WRITE_NOTIFI 119                /* w */
#define ADD_READ_NOTIFI 82                  /* R */
#define DEL_READ_NOTIFI 114                 /* r */
#define DEL_ALL_NOTIFI 115                  /* s */

#define WHITELIST_NODE_MIN 5
#define WHITELIST_NODE_MAX 4090
#define WHITELIST_LIMIT 128
 ...
 * < 0: error code
 */
static int add_execve_exe_whitelist(char *data)
{
    struct whitelist_node *node;
    int rc = 0;
    ...
    node = smith_kzalloc(sizeof(struct whitelist_node), GFP_KERNEL);
    if (!node)
        return -ENOMEM;
    node->data = data;

    write_lock(&exe_whitelist_lock);
    rc = insert_rb(&execve_exe_whitelist, node);
    if (!rc) {
        execve_exe_whitelist_limit++;
        write_unlock(&exe_whitelist_lock);
    } else {
        write_unlock(&exe_whitelist_lock);
        printk(KERN_INFO "[ELKEID] add_execve_exe_whitelist: already added.\n");
        smith_kfree(node);
    }
...
static void del_execve_exe_whitelist(char *data)
{
    ...
    write_lock(&exe_whitelist_lock);
    node = search_rb(&execve_exe_whitelist, data);
    if (node) {
        rb_erase(&node->node, &execve_exe_whitelist);
        execve_exe_whitelist_limit--;
    }
    write_unlock(&exe_whitelist_lock);
...
static void print_all_execve_whitelist(void)
{
    ...
    for (node = rb_first(&execve_exe_whitelist); node; node = rb_next(node)) {
        ...
        printk("[ELKEID DEBUG] execve_whitelist:%s \n", data->data);
    }
 ...
 * < 0: error code
 */
static int add_execve_argv_whitelist(char *data)
{
    struct whitelist_node *node;
    int rc = 0;
    ...
    rc = insert_rb(&execve_argv_whitelist, node);
    if (!rc) {
        execve_argv_whitelist_limit++;
        write_unlock(&argv_whitelist_lock);
    } else {
        write_unlock(&argv_whitelist_lock);
        printk(KERN_INFO"[ELKEID] add_execve_argv_whitelist: already added.\n");
        smith_kfree(node);
    }
...
static void del_execve_argv_whitelist(char *data)
{
    struct whitelist_node *node;

    write_lock(&argv_whitelist_lock);
    node = search_rb(&execve_argv_whitelist, data);
    if (node) {
        rb_erase(&node->node, &execve_argv_whitelist);
        execve_argv_whitelist_limit--;
    }
    write_unlock(&argv_whitelist_lock);
...
static void print_all_argv_whitelist(void)
{
    ...
    for (node = rb_first(&execve_argv_whitelist); node;
        ...
        struct whitelist_node *data =
        container_of(node, struct whitelist_node, node);
        printk("[ELKEID DEBUG] argv_whitelist:%s \n", data->data);
    }
                            ...
                            size_t len, loff_t * off)
{
    ...
    if (flag == ADD_EXECVE_EXE_SHITELIST &&
        execve_exe_whitelist_limit > WHITELIST_LIMIT) {
        return len;
    }
    if (flag == ADD_EXECVE_ARGV_SHITELIST &&
        execve_argv_whitelist_limit > WHITELIST_LIMIT) {
        return len;
    }
    ...
    switch (flag) {
        case ADD_EXECVE_EXE_SHITELIST:
            if (!add_execve_exe_whitelist(smith_strim(data_main)))
                data_main = NULL;
            break;
        ...
        case ADD_EXECVE_ARGV_SHITELIST:
            if (!add_execve_argv_whitelist(smith_strim(data_main)))
                data_main = NULL;
            break;

        case DEL_EXECVE_ARGV_SHITELIST:
            del_execve_argv_whitelist(strim(data_main));
            break;
